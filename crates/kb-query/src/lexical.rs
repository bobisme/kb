...
impl LexicalIndex {
    ...
    pub fn search(&self, query: &str, top_k: usize) -> Vec<SearchResult> {
        ...
        let mut scored: Vec<(usize, &LexicalEntry)> = self
            .entries
            .iter()
            .filter_map(|entry| {
...
fn score_entry(entry: &LexicalEntry, query_tokens: &[String]) -> usize {
    ...
    query_tokens
        .iter()
        .map(|token| {
            count_occurrences(&title_tokens, token) * WEIGHT_TITLE
                + count_occurrences(&alias_tokens, token) * WEIGHT_ALIAS
                + count_occurrences(&heading_tokens, token) * WEIGHT_HEADING
                + count_occurrences(&summary_tokens, token) * WEIGHT_SUMMARY
        })
        .sum()
}
