...
const AUTO_DOWNLOAD_ENV: &str = "KB_SEMANTIC_AUTO_DOWNLOAD";
...
fn default_model_path() -> Result<PathBuf> {
    Ok(MiniLmBackend::cache_root()?.join(MODEL_FILENAME))
}

fn default_tokenizer_path() -> Result<PathBuf> {
    Ok(MiniLmBackend::cache_root()?.join(TOKENIZER_FILENAME))
}
